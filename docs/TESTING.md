...
- recuperação de pendente anterior a 2.000 execuções concluídas, também exercitada em SQLite.
...
diff Lambda sanitizado e confirmação de produção ao repetir uma execução. O adaptador do
canvas tem regressão para valores antigos de widget e propriedades de arestas criadas no
navegador. O percurso Chromium aguarda o estado `notRunning` do Streamlit antes de prosseguir,
confere o grafo no banco e exige SUCCESS também na reexecução.
