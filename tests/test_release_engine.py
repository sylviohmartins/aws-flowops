...
class ReleaseEngineTests(unittest.TestCase):
    ...
    def test_aws_batch_envelopes_and_s3_partial_failure(self) -> None:
        ...
        with self.assertRaises(ProviderError) as error:
            action.execute(cases[-1][1], fixtures.ActionContext("exec", "delete", fixtures.AWSContext(), False))
    ...
    def test_batch_and_iteration_limit_before_any_effect(self) -> None:
        ...
        each = Node(id="each", action="core.for_each", config={
            "items": [{"records": [1, 2]}, {"records": [3, 4]}],
            "template": "{{ item }}", "action": "test.action",
        })
        for index, node in enumerate([Node(id="call", action="test.action", config={"records": [1, 2, 3, 4]}), each]):
    ...
    def test_iteration_approval_covers_all_items_without_replay(self) -> None:
        ...
        f.engine.approve(execution_id, "each", approval["digest"], f.other, approved=True, reason="Review both items")
    ...
    def test_manual_intervention_records_reconciliation_without_replaying_call(self) -> None:
        f = self.fixture
        f.action.failures = 1
        execution_id = f.submit(f.book([Node(id="call", action="test.action", failure_policy="MANUAL_INTERVENTION")]))
        self.assertEqual(f.engine.execute(execution_id).status, Status.WAITING_APPROVAL)
        approval = f.engine.store.pending_approvals()[0]
        with self.assertRaises(PolicyViolation):
            f.engine.approve(execution_id, "call", approval["digest"], f.actor, approved=True, reason="self")
        f.engine.approve(execution_id, "call", approval["digest"], f.other, approved=True, reason="Reconciled externally, ticket OPS-9")
        result = f.engine.execute(execution_id)
        self.assertEqual(result.status, Status.SUCCESS)
        self.assertEqual(result.node_outputs["call"], {"manual_intervention": True, "reconciled": True})
    ...
    def test_success_checkpoint_contains_result_at_first_terminal_write(self) -> None:
        ...
        def crash_after_success(execution: Any, node_id: str, status: Status, detail: dict[str, Any]) -> None:
        ...
        with patch.object(f.engine.store, "checkpoint", side_effect=crash_after_success), self.assertRaises(KeyboardInterrupt):
            f.engine.execute(execution_id)
    ...
    def test_iteration_checkpoint_collisions_fail_validation(self) -> None:
        ...
        for ids in (("each", "each__0"), ("a" * 49, "a" * 48 + "b")):
            book = f.book([Node(id=value, action="core.for_each", config={"items": [], "template": {}}) for value in ids])
