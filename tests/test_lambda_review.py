...
class LambdaReviewTests(unittest.TestCase):
    def setUp(self) -> None:
        self.current: dict[str, Any] = {
            "Configuration": {"FunctionName": "payment-processor", "PackageType": "Zip", "Runtime": "python3.12", "RevisionId": "revision-1", "Environment": {"Variables": {"DB_URL": "never-display", "unchanged": "hidden"}}},
    ...
    def test_configuration_review_hides_environment_values_and_download_urls(self) -> None:
        result = change_preview(self.current, "lambda.update_function_configuration", {"Timeout": 15, "Environment": {"Variables": {"DB_URL": "new-secret", "unchanged": "hidden"}}})
    ...
    def test_zip_s3_image_alias_and_version_review(self) -> None:
        ...
        zip_review = change_preview(self.current, "lambda.update_function_code", {"ZipFile": {"base64": content}})
        self.assertEqual(zip_review["proposed"]["ProposedArtifact"]["ZipFile"]["bytes"], 18)
        self.assertNotIn(content, str(zip_review))
        s3 = change_preview(self.current, "lambda.update_function_code", {"S3Bucket": "artifacts", "S3Key": "payment.zip", "S3ObjectVersion": "immutable-version"})
        ...
        image = change_preview(self.current, "lambda.update_function_code", {"ImageUri": "account.dkr.ecr.region.amazonaws.com/repo@sha256:123"})
        self.assertIn("ImageUri", image["diff"])
        for operation in ("create_alias", "update_alias", "delete_alias"):
            result = change_preview(self.current, f"lambda.{operation}", {"Name": "live", "FunctionVersion": "2"})
            ...
            self.assertEqual(result["revision_id"], "alias-1" if operation == "update_alias" else None)
        published = change_preview(self.current, "lambda.publish_version", {"Description": "Release"})
        self.assertIn("PublishVersion", published["proposed"])

    def test_invalid_artifacts_and_unresolved_functions_fail_closed(self) -> None:
        for config in ({}, {"ImageUri": "image", "S3Bucket": "b", "S3Key": "k"}, {"ImageUri": "image"}, {"ZipFile": {"base64": "%%%"}}):
        ...
        for config in ({}, {"FunctionName": "{{ params.function }}"}):
            with self.assertRaises(WorkflowValidationError):
                review_lambda(ActionRegistry(), Identity(id="admin", roles=["ADMIN"]), AWSContext(), "lambda.update_function_code", config)
        with self.assertRaises(WorkflowValidationError):
            review_lambda(ActionRegistry(), Identity(id="admin", roles=["ADMIN"]), AWSContext(), "lambda.invoke", {})
        with self.assertRaises(AuthorizationError):
            review_lambda(ActionRegistry(), Identity(id="viewer", roles=["VIEWER"]), AWSContext(), "lambda.update_function_code", {})
    ...
    def test_demo_current_state_and_backend_release(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = review_lambda(build_registry(backend), Identity(id="admin", roles=["ADMIN"]), AWSContext(), "lambda.update_function_configuration", {"FunctionName": "payment-processor", "Timeout": 10})
