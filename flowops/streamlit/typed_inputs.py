...
def render_typed_inputs(ui: FlowOpsUI, book: Runbook, node: Node, revision: int) -> None:
    ...
    with st.expander("Typed inputs", expanded=False):
        fields = st.multiselect("Input fields", list(properties), default=[name for name in properties if name in node.config or name in required], key=f"flowops:typed-fields:{book.id}:{node.id}", disabled=not editable)
        values: dict[str, Any] = {}
        with st.form(f"flowops:typed:{book.id}:{node.id}:{digest(node.config)}"):
            for name in fields:
                ...
                if isinstance(value, str) and "{{" in value:
                    values[name] = st.text_input(label, value=value, help=help_text, disabled=not editable)
                elif field.get("enum"):
                    options = list(field["enum"])
                    values[name] = st.selectbox(label, options, index=options.index(value) if value in options else 0, help=help_text, disabled=not editable)
                elif kind in {"integer", "number"}:
                    values[name] = st.number_input(label, value=value, step=1 if kind == "integer" else 0.1, help=help_text, disabled=not editable)
                elif kind == "boolean":
                    values[name] = st.checkbox(label, value=bool(value), help=help_text, disabled=not editable)
                elif kind == "string":
                    values[name] = st.text_input(label, value=value or "", help=help_text, disabled=not editable)
                else:
                    raw = st.text_area(label + " (JSON)", value=json.dumps(value if value is not None else {} if kind == "object" else [] if kind == "array" else ""), help=help_text, disabled=not editable)
