...
ROLE_PERMISSIONS = {
    ...
    "APPROVER": {
        "runbook.read",
        "runbook.approve.dev",
        "runbook.approve.staging",
        "runbook.approve.production",
    },
    "ADMIN": {"*"},
}
...
class PolicyEngine:
    ...
    def execution(self, execution: Execution) -> None:
        ...
        if (
            not execution.dry_run
            and execution.aws_context.environment == "production"
            and not execution.reason.strip()
        ):
    ...
    def action(self, execution: Execution, metadata: Metadata, affected: int = 1) -> bool:
        if metadata.provider == "core":
            return False
        require(
            execution.actor, "aws.read" if metadata.read_only else "aws.write", execution.snapshot
        )
        ...
        if execution.dry_run or metadata.read_only:
            return False
        return (
            execution.aws_context.environment == "production"
            or metadata.risk == Risk.CRITICAL
            or affected > self.approval_threshold
        )
