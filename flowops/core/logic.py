...
def logic(
    action: str, config: dict[str, Any], scope: dict[str, Any], limit: int = 1000
) -> tuple[Any, str]:
    ...
    if action == "core.switch":
        ...
        branch = next(
            (str(label) for label, value in cases.items() if compare(config["value"], "eq", value)),
            "default",
        )
    ...
    if action in {"core.filter", "core.map", "core.for_each", "core.batch"}:
        if not isinstance(items, list) or len(items) > limit:
            raise WorkflowValidationError(
                f"Collection must be an array with at most {limit} items."
            )
        if action == "core.filter":
            return {
                "items": [
                    ...
                    for item in items
                    if compare(
                        lookup(f"item.{config['path']}", {"item": item}),
                        config.get("operator", "eq"),
                        config["value"],
                    )
                ]
            }, "default"
        if action in {"core.map", "core.for_each"}:
            return {
                "items": [resolve(config["template"], scope | {"item": item}) for item in items]
            }, "default"
        size = config["size"]
