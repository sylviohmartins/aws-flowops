...
SUPPORTED = {"dynamodb.list_tables", "dynamodb.describe_table", "dynamodb.get_item", "dynamodb.query", "dynamodb.scan", "dynamodb.update_item", "sqs.list_queues", "sqs.send_message", "sqs.get_queue_attributes", "lambda.list_functions", "lambda.get_function_configuration", "lambda.invoke", "sns.list_topics", "sns.publish", "s3.list_buckets", "s3.list_objects_v2", "s3.put_object", "s3.get_object"}
...
class DemoBackend:
    ...
    def __init__(self, repository: Repository):
        ...
        with repository.transaction() as db:
            ...
            if exists is None:
                db.execute("INSERT INTO resource_bindings VALUES (?,?)", ("demo-state", canonical(self.initial_state())))

    @staticmethod
    def initial_state() -> dict[str, Any]:
        return {"payments": {key: {"paymentId": {"S": key}, "status": {"S": "PROCESSING"}} for key in ["12345", "23456", "34567"]}, "messages": [], "objects": {}}

    def reset(self) -> None:
        with self.repository.transaction() as db:
            db.execute("UPDATE resource_bindings SET body=? WHERE id='demo-state'", (canonical(self.initial_state()),))
    ...
    def invoke(self, service: str, operation: str, parameters: dict[str, Any], context: ActionContext, limits: Limits) -> Any:
        ...
        if key not in SUPPORTED:
            raise ProviderError(f"DemoUnsupportedOperation: {key}; select a configured real AWS context")
        with self.repository.transaction() as db:
            state = json.loads(db.execute("SELECT body FROM resource_bindings WHERE id='demo-state'").fetchone()[0])
            result = self._call(key, parameters, state, context, limits)
            db.execute("UPDATE resource_bindings SET body=? WHERE id='demo-state'", (canonical(state),))
        return result | {"ResponseMetadata": {"RequestId": f"demo-{context.execution_id}-{context.node_id}", "HTTPStatusCode": 200}, "_demo": True}

    def _call(self, key: str, p: dict[str, Any], state: dict[str, Any], context: ActionContext, limits: Limits) -> dict[str, Any]:
        ...
        if key.startswith("dynamodb."):
            ...
            if key == "dynamodb.describe_table":
                return {"Table": {"TableName": "payments", "KeySchema": [{"AttributeName": "paymentId", "KeyType": "HASH"}], "TableStatus": "ACTIVE"}}
            if key in {"dynamodb.query", "dynamodb.scan"}:
                ...
                return {"Items": items[:min(p.get("Limit", limits.max_items), limits.max_items)], "Count": len(items)}
            ...
            if p.get("ConditionExpression") != "#s = :expected" or p.get("UpdateExpression") != "SET #s = :next" or p.get("ExpressionAttributeNames") != {"#s": "status"}:
        ...
        if key == "sqs.send_message":
            ...
            return {"MessageId": f"demo-message-{len(state['messages'])}", "MD5OfMessageBody": "demo-only"}
        ...
        if key == "lambda.get_function_configuration":
            return {"FunctionName": p["FunctionName"], "Runtime": "python3.12", "PackageType": "Zip", "RevisionId": "demo-revision"}
        if key == "lambda.invoke":
            return {"StatusCode": 200, "Payload": {"accepted": True, "payload": json.loads(p.get("Payload", "{}"))}}
