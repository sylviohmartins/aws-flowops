...
REVIEW_ACTIONS = {
    "lambda.update_function_configuration", "lambda.update_function_code",
    "lambda.publish_version", "lambda.create_alias", "lambda.update_alias", "lambda.delete_alias",
}


def _display(value: Any) -> Any:
    if isinstance(value, dict):
        return {key: ({name: "[REDACTED]" for name in item} if key == "Variables" and isinstance(item, dict) else _display(item))
                for key, item in value.items() if key not in {"Location", "ResponseMetadata"}}
    if isinstance(value, list):
        return [_display(item) for item in value]
    return value


def change_preview(current: dict[str, Any], action_id: str, config: dict[str, Any]) -> dict[str, Any]:
    if action_id not in REVIEW_ACTIONS:
        raise WorkflowValidationError("This operation has no Lambda change review.")
    state = copy.deepcopy(current)
    proposed = copy.deepcopy(current)
    parameters = {key: value for key, value in config.items() if key not in {"FunctionName", "_flowops"}}
    ...
    if action_id.endswith("update_function_configuration"):
        ...
        if "Environment" in parameters:
            ...
            proposed["EnvironmentVariablesChanged"] = sorted(key for key in old.keys() | new.keys() if old.get(key) != new.get(key))
        ...
        if sum((image, zipped, stored)) != 1:
            raise WorkflowValidationError("Choose exactly one Lambda artifact: ZipFile, S3 bucket/key, or ImageUri.")
        if (configuration.get("PackageType") == "Image") != image:
            raise WorkflowValidationError("The artifact must match the current Lambda package type.")
        if zipped:
            raw = parameters["ZipFile"]
            try:
                content = base64.b64decode(raw["base64"], validate=True)
            except (KeyError, TypeError, ValueError) as exc:
                raise WorkflowValidationError("ZipFile must use the base64 binary envelope.") from exc
            parameters["ZipFile"] = {"sha256": hashlib.sha256(content).hexdigest(), "bytes": len(content)}
        ...
        existing = next((alias for alias in aliases if alias.get("Name") == name), {})
    ...
    safe_current, safe_proposed = bounded_output(_display(state)), bounded_output(_display(proposed))
    ...
    return {"current": safe_current, "proposed": safe_proposed, "diff": diff, "revision_id": revision}


def review_lambda(registry: ActionRegistry, user: Identity, aws: AWSContext, action_id: str, config: dict[str, Any]) -> dict[str, Any]:
    require(user, "aws.read")
    if action_id not in REVIEW_ACTIONS:
        raise WorkflowValidationError("This operation has no Lambda change review.")
    name = config.get("FunctionName")
    if not isinstance(name, str) or not name or "{{" in name:
        raise WorkflowValidationError("Set a concrete FunctionName before loading the current state.")
    ...
    try:
        ...
        for operation, field in (("list_aliases", "Aliases"), ("list_versions_by_function", "Versions")):
            reader = registry.get(f"lambda.{operation}")
            current[field] = reader.execute(parameters | {"_flowops": {"paginate": True, "max_items": 100}}, context).get(field, [])
