    ...
    if isinstance(parameters, dict):
        for key, value in parameters.items():
            if key == "QueueUrl" and isinstance(value, str):
                ...
                if parsed.scheme != "https" or parsed.hostname != f"sqs.{context.region}.{suffix}" or parsed.port not in (None, 443) or parsed.username or parsed.password or parsed.query or parsed.fragment or not parsed.path.startswith(f"/{context.account_id}/"):
                    raise PolicyViolation("Queue URL must belong to the selected AWS account and region.")
            if isinstance(value, str) and value.startswith("arn:"):
                parts = value.split(":", 5)
                if len(parts) != 6 or parts[4] not in {"", context.account_id} or parts[3] not in {"", context.region}:
                    raise PolicyViolation("Cross-account or cross-region resource reference denied.")
    ...
    def _client(self, service: str, context: ActionContext) -> Any:
        ...
        with self.lock:
            ...
            if entry is None or entry[1] < datetime.now(UTC) + timedelta(minutes=5):
                session = (self.session_factory or boto3.Session)(profile_name=aws.profile, region_name=aws.region)
                expires = datetime.now(UTC) + timedelta(hours=1)
                sdk_config = Config(connect_timeout=5, read_timeout=30, retries={"mode": "standard", "total_max_attempts": 1}, user_agent_extra=f"flowops/{context.execution_id}", ignore_configured_endpoint_urls=True)
                if aws.role_arn:
                    role: dict[str, Any] = {"RoleArn": aws.role_arn, "RoleSessionName": f"flowops-{context.execution_id[:40]}", "DurationSeconds": 3600}
                    if aws.external_id:
                        role["ExternalId"] = aws.external_id
                    credentials = session.client("sts", config=sdk_config).assume_role(**role)["Credentials"]
                    session = boto3.Session(aws_access_key_id=credentials["AccessKeyId"], aws_secret_access_key=credentials["SecretAccessKey"], aws_session_token=credentials["SessionToken"], region_name=aws.region)
                ...
                self.clients = {key: client for key, client in self.clients.items() if key[0] != context.execution_id}
                entry = session, expires
            key = (context.execution_id, service)
            if key not in self.clients:
                self.clients[key] = entry[0].client(service, config=Config(connect_timeout=5, read_timeout=30, retries={"mode": "standard", "total_max_attempts": 1}, user_agent_extra=f"flowops/{context.execution_id}", ignore_configured_endpoint_urls=True))
    ...
    def invoke(self, service: str, operation: str, parameters: dict[str, Any], context: ActionContext, limits: Limits) -> Any:
        ...
        try:
            ...
            if service == "s3" and "Bucket" in parameters:
                ...
                if "CopySource" in parameters and "ExpectedSourceBucketOwner" in model.input_shape.members:
            ...
            if limits.paginate:
                ...
                iterator = client.get_paginator(operation).paginate(**parameters, PaginationConfig={"MaxItems": limits.max_items})
                result: dict[str, Any] = {}
                pages = 0
                for page in iterator:
                    ...
                    if pages >= limits.max_pages:
                        break
                result["_flowops"] = {"pages": pages, "bounded": True, "possibly_more": pages >= limits.max_pages, "resume_token": iterator.resume_token}
            else:
            ...
            error = ProviderError(code, retryable=code in {"Throttling", "ThrottlingException", "ProvisionedThroughputExceededException", "ServiceUnavailable"})
            error.details = {"request_id": exc.response.get("ResponseMetadata", {}).get("RequestId")}
            ...
            LOG.info("aws_api_call", extra={"execution_id": context.execution_id, "node_id": context.node_id, "service": service, "action": operation, "account": context.aws.account_id, "region": context.aws.region, "duration_seconds": time.monotonic() - start})
