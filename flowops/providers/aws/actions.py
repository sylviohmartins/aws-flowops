...
class AWSAction:
    def __init__(self, spec: Spec, backend: Backend, catalog: ModelCatalog | None = None):
        ...
        operation_name = model.name if model else "".join(part.title() for part in spec.operation.split("_"))
        self.metadata = Metadata(spec.id, "aws", spec.service, spec.operation, f"AWS {spec.service} {operation_name}", risk=spec.risk, read_only=spec.read_only, idempotent=spec.idempotent, required_permissions=IAM_OVERRIDES.get(spec.id, (f"{spec.service}:{operation_name}",)), input_schema=catalog.schema(model.input_shape) if catalog else {"type": "object"}, output_schema=catalog.schema(model.output_shape) if catalog else {"type": "object"})

    def prepare(self, config: dict[str, Any]) -> tuple[dict[str, Any], Limits]:
        parameters = dict(config)
        control = parameters.pop("_flowops", {})
        if not isinstance(control, dict) or control.keys() - {"max_items", "max_pages", "max_bytes", "paginate"}:
            raise WorkflowValidationError("Invalid FlowOps pagination options.")
        limits = Limits(**control)
        if type(limits.max_items) is not int or not 1 <= limits.max_items <= 1000 or type(limits.max_pages) is not int or not 1 <= limits.max_pages <= 10 or type(limits.max_bytes) is not int or not 1024 <= limits.max_bytes <= 1048576 or type(limits.paginate) is not bool:
        ...
        for key in ("MessageBody", "Message", "Payload"):
            if key in parameters and isinstance(parameters[key], (dict, list)):
                parameters[key] = json.dumps(parameters[key], ensure_ascii=False, separators=(",", ":"))
        if self.spec.operation == "send_message_batch":
            parameters["Entries"] = [dict(entry) | {"MessageBody": json.dumps(entry["MessageBody"], ensure_ascii=False) if isinstance(entry.get("MessageBody"), (dict, list)) else entry.get("MessageBody")} for entry in parameters.get("Entries", [])]
    ...
    def preview(self, config: dict[str, Any], context: ActionContext) -> Any:
        ...
        return {"simulation": True, "native_dry_run": False, "action": self.spec.id, "parameters": config, "limits": {"max_items": limits.max_items, "max_pages": limits.max_pages}, "note": "Mutation was not called. This is a plan, not a confirmed AWS result."}

    def execute(self, config: dict[str, Any], context: ActionContext) -> Any:
        parameters, limits = self.prepare(config)
        result = self.backend.invoke(self.spec.service, self.spec.operation, parameters, context, limits)
        if isinstance(result, dict) and (result.get("UnprocessedItems") or result.get("UnprocessedKeys") or result.get("Failed") or result.get("FunctionError") or any(isinstance(r, dict) and r.get("Error") for r in result.get("Responses", []))):
...
def register_generic(registry: ActionRegistry, catalog: ModelCatalog, backend: Backend, service: str, operation: str, allowlist: set[str]) -> str:
