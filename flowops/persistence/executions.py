...
class ExecutionStore:
    ...
    def create(self, execution: Execution, token: str) -> Execution:
        ...
        with self.repository.transaction() as db:
            row = db.execute(
                "SELECT body,request_digest FROM executions WHERE token=?", (token,)
            ).fetchone()
            if row:
    ...
    def history(self, limit: int = 200) -> list[Execution]:
        with self.repository.transaction() as db:
            rows = db.execute(
                "SELECT body FROM executions ORDER BY created_at DESC LIMIT ?", (min(limit, 2000),)
            ).fetchall()
    ...
    def claim(self, execution_id: str) -> bool:
        with self.repository.transaction() as db:
            row = db.execute(
                "SELECT body FROM executions WHERE id=? AND status=?",
                (execution_id, Status.PENDING.value),
            ).fetchone()
            if row is None:
                return False
            execution = Execution.model_validate_json(row["body"])
            ...
            changed = db.execute(
                "UPDATE executions SET status=?,body=?,revision=revision+1 WHERE id=? AND status=?",
                (
    ...
    def save(self, execution: Execution) -> None:
        with self.repository.transaction() as db:
            db.execute(
                "UPDATE executions SET status=?,body=?,revision=revision+1 WHERE id=?",
                (execution.status.value, execution.model_dump_json(), execution.id),
            )
    ...
    def checkpoint(
        self, execution: Execution, node_id: str, status: Status, detail: dict[str, Any]
    ) -> None:
    ...
    def nodes(self, execution_id: str) -> dict[str, dict[str, Any]]:
        with self.repository.transaction() as db:
            rows = db.execute(
                "SELECT node_id,status,body FROM node_executions WHERE execution_id=?",
                (execution_id,),
            ).fetchall()
    ...
    def cancelled(self, execution_id: str) -> bool:
        with self.repository.transaction() as db:
            row = db.execute(
                "SELECT cancel_requested FROM executions WHERE id=?", (execution_id,)
            ).fetchone()
    ...
    def approval(
        ...
        with self.repository.transaction() as db:
            row = db.execute(
                ...
                (execution.id, node_id, input_digest),
            ).fetchone()
            if row:
                return str(row[0])
            db.execute(
    ...
    def decide(
        ...
        with self.repository.transaction() as db:
            changed = db.execute(
                "UPDATE approvals SET decision=?,approver=?,reason=?,decided_at=? WHERE execution_id=? AND node_id=? AND digest=? AND decision='PENDING'",
                (
            ...
            db.execute(
                "UPDATE executions SET status=?,body=? WHERE id=?",
                (execution.status.value, execution.model_dump_json(), execution_id),
            )
            self.repository.event(
                db,
                actor,
